...
# Validate command
case $COMMAND in
    train-local|train-server|test-local|test-server|vis-local|vis-server)
        ;;
    *)
...
read NGF NDF PATCH_X PATCH_Y PATCH_Z GENERATOR DISCRIMINATOR <<< "$MODEL_PARAMS"
...
case $COMMAND in
    ...
    test-*)
        ...
        EXPERIMENT_NAME="${THIRD_ARG}_${CONFIG_NAME}_G${GENERATOR}_D${DISCRIMINATOR}_ngf${NGF}_ndf${NDF}_patch${PATCH_X}_${PATCH_Y}_${PATCH_Z}"
